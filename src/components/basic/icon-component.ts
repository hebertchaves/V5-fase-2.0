...
export async function processIconComponent(node: QuasarNode, settings: PluginSettings): Promise<FrameNode> {
  ...
  if (props.size) {
    if (props.size.endsWith('em')) {
      // Converter em para pixels (assumindo 1em = 16px)
      const emSize = parseFloat(props.size);
      iconSize = Math.round(emSize * 16);
    } else if (props.size.endsWith('px')) {
      // Extrair valor numérico
      iconSize = parseInt(props.size);
    } else {
      switch (props.size) {
  ...
  // Tentar criar o ícone com diferentes fontes
  const fonts = [
  ...
  for (const font of fonts) {
    try {
      ...
      const unicodeChar = getIconUnicode(library, normalizeIconName(iconName));
      textNode.characters = unicodeChar;
      
      // Aplicar cor
      if (props.color && quasarColors[props.color]) {
        textNode.fills = [{ type: 'SOLID', color: quasarColors[props.color] }];
      } else if (node.parentContext?.attributes?.color && quasarColors[node.parentContext.attributes.color]) {
        // Herdar cor do pai (ex: botão)
        textNode.fills = [{ type: 'SOLID', color: quasarColors[node.parentContext.attributes.color] }];
      }
      ...
      console.warn(`Não foi possível usar a fonte ${font.family}: ${error}`);
      continue;
    }
  }
  
  // Fallback se nenhuma fonte puder ser carregada
  if (!iconCreated) {
    console.error('Nenhuma fonte de ícone disponível, usando placeholder');
    
    // Criar um placeholder visual
