...
export function getIconLibrary(iconName: string): string {
  ...
  if (iconName.startsWith('fa-') || 
      ...
      iconName.match(/^fa[srlb]?-/i)) {
    return 'fontawesome';
  } 
  else if (iconName.startsWith('ion-') || iconName.startsWith('ios-') || iconName.startsWith('md-')) {
    return 'ionicons';
  }
...
export function getIconUnicode(library: string, iconName: string): string {
  ...
  if (library === 'material' && materialIconsMap[normalizedName]) {
    return materialIconsMap[normalizedName];
  }
  
  // Tentar encontrar ícones comuns
...
'text_rotation_none': '\uE93D',
  ...
  if (commonIcons[normalizedName]) {
    return commonIcons[normalizedName];
  }
